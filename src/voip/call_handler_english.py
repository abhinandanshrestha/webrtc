...
import sys, os
from llm_handler_english import *
...
class CallHandler:
    ...
    def tts(self):
        ...
        while True:
            ...
            if self.tts_queue and not self.tts_queue.empty():
                ...
                if self.state==1:
                    if llm_output=='yes':
                        audio_array=get_audio_bytes('/home/oem/webrtc/src/callbot/audios/e2_yes.wav')
                        self.logs['TTS Plays: e2_yes.wav']=datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        requests.post(self.call_logs_url, params={'caller_id': self.caller_id,'event_type': 'tts pushes','event_detail': 'e2_yes.wav'}, headers={'accept': 'application/json'})   
                        self.state+=1
                    elif llm_output=='no':
                        audio_array=get_audio_bytes('/home/oem/webrtc/src/callbot/audios/e2_no.wav')
                        requests.post(self.call_logs_url, params={'caller_id': self.caller_id,'event_type': 'tts pushes','event_detail': 'e2_no.wav'}, headers={'accept': 'application/json'})
                        self.logs['TTS Plays: e2_no.wav']=datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        self.state=self.out_state
                    elif llm_output=='repeat':
                        audio_array=get_audio_bytes('/home/oem/webrtc/src/callbot/audios/e1.wav')
                        self.logs['TTS Plays: e1.wav']=datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        requests.post(self.call_logs_url, params={'caller_id': self.caller_id,'event_type': 'tts pushes','event_detail': 'e1.wav'}, headers={'accept': 'application/json'})
                        self.state=1
                elif self.state==2:
                    if llm_output=='yes':
                        audio_array=get_audio_bytes('/home/oem/webrtc/src/callbot/audios/e3_yes.wav')
                        self.logs['TTS Plays: e3_yes.wav']=datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        requests.post(self.call_logs_url, params={'caller_id': self.caller_id,'event_type': 'tts pushes','event_detail': 'e3_yes.wav'}, headers={'accept': 'application/json'})
                        self.state+=1
                    elif llm_output=='no':
                        audio_array=get_audio_bytes('/home/oem/webrtc/src/callbot/audios/e3_no.wav')
                        self.logs['TTS Plays: e3_no.wav']=datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        requests.post(self.call_logs_url, params={'caller_id': self.caller_id,'event_type': 'tts pushes','event_detail': 'e3_no.wav'}, headers={'accept': 'application/json'})
                        self.state=self.out_state
                    elif llm_output=='repeat':
                        audio_array=get_audio_bytes('/home/oem/webrtc/src/callbot/audios/e2_yes.wav')
                        self.logs['TTS Plays: e2_yes.wav']=datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    ...
                    if llm_output=='yes':
                        audio_array=get_audio_bytes('/home/oem/webrtc/src/callbot/audios/e4_yes.wav')
                        self.logs['TTS Plays: e4_yes.wav']=datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        requests.post(self.call_logs_url, params={'caller_id': self.caller_id,'event_type': 'tts pushes','event_detail': 'e4_yes.wav'}, headers={'accept': 'application/json'})
                        self.state+=1
                    elif llm_output=='no':
                        audio_array=get_audio_bytes('/home/oem/webrtc/src/callbot/audios/e4_no.wav')
                        requests.post(self.call_logs_url, params={'caller_id': self.caller_id,'event_type': 'tts pushes','event_detail': 'e4_no.wav'}, headers={'accept': 'application/json'})
                        self.logs['TTS Plays: e4_no.wav']=datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        self.state=self.out_state
                    elif llm_output=='repeat':
                        audio_array=get_audio_bytes('/home/oem/webrtc/src/callbot/audios/e3_yes.wav')
                        self.logs['TTS Plays: e3_yes.wav']=datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    ...
                    if llm_output=='yes':
                        audio_array=get_audio_bytes('/home/oem/webrtc/src/callbot/audios/e5.wav')
                        self.logs['TTS Plays: e5_yes.wav']=datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        requests.post(self.call_logs_url, params={'caller_id': self.caller_id,'event_type': 'tts pushes','event_detail': 'e5.wav'}, headers={'accept': 'application/json'})
    ...
    def handle_call(self, call):
        ...
        try:
            ...
            call.write_audio(get_audio_bytes('/home/oem/webrtc/src/callbot/audios/e1.wav'))
